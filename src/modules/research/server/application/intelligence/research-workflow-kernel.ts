...
import { buildQuickResearchTaskContract } from "~/modules/research/server/domain/workflow/research";
import {
...
} from "~/modules/research/server/domain/workflow/research-schemas";
import type {
  DeepSeekClient,
  DeepSeekMessage,
} from "~/modules/research/server/infrastructure/intelligence/deepseek-client";
...
function acceptanceCriteriaFor(capability: ResearchUnitCapability) {
  switch (capability) {
    ...
    case "industry_search":
      return ["梳理竞争格局或产业链位置。", "返回能回答关键战略问题的证据。"];
    case "page_scrape":
      return ["从页面中提取可核验的一手事实。", "保留来源链接，便于后续引用。"];
    case "financial_pack":
      return [
        "在有股票代码时返回结构化财务证据。",
        "若未取到数据包，需要明确说明数据缺口。",
...
function withUnitMetadata(
  ...
  return {
    ...
    objective:
      unit.objective.trim() || unit.title.trim() || `研究单元 ${index + 1}`,
