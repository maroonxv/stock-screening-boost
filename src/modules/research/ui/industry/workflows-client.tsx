...
import { buildQuickResearchStartInput } from "~/modules/research/ui/industry/quick-research-form";
import { WorkflowVisualizationPanel } from "~/modules/research/ui/industry/workflow-visualization-panel";
import {
...
export function WorkflowsClient() {
  ...
  const runsQuery = api.research.runs.listRuns.useQuery({
    limit: 20,
    templateCode: QUICK_RESEARCH_TEMPLATE_CODE,
  });

  const startMutation = api.research.runs.startQuickResearch.useMutation({
    onSuccess: async (result) => {
      await utils.research.runs.listRuns.invalidate({
        limit: 20,
        templateCode: QUICK_RESEARCH_TEMPLATE_CODE,
      });
