...
import { buildResearchDigest } from "~/modules/research/ui/industry/research-view-models";
...
} from "~/modules/research/ui/spaces/space-view-models";
...
export function SpaceDetailClient({ spaceId }: SpaceDetailClientProps) {
  ...
  const detailQuery = api.research.spaces.getDetail.useQuery(
    { spaceId },
    { refetchOnWindowFocus: false },
  );
  const runLinksQuery = api.research.spaces.listRunLinks.useQuery(
    {
      spaceId,
      limit: 50,
      offset: 0,
  ...
  const updateMetaMutation = api.research.spaces.updateMeta.useMutation({
    onSuccess: async () => {
      setNotice("空间元信息已更新。");
      await utils.research.spaces.getDetail.invalidate({ spaceId });
    },
  });
  const updateBriefMutation = api.research.spaces.updateBrief.useMutation({
    onSuccess: async () => {
      setNotice("Space brief 已更新。");
      await utils.research.spaces.getDetail.invalidate({ spaceId });
    },
  });
  const unlinkWatchlistMutation =
    api.research.spaces.unlinkWatchlist.useMutation({
      onSuccess: async () => {
        await utils.research.spaces.getDetail.invalidate({ spaceId });
      },
    });
  const unlinkStockMutation = api.research.spaces.unlinkStock.useMutation({
    onSuccess: async () => {
      await utils.research.spaces.getDetail.invalidate({ spaceId });
    },
  });
  const removeRunMutation = api.research.spaces.removeRun.useMutation({
    onSuccess: async () => {
      await Promise.all([
        utils.research.spaces.getDetail.invalidate({ spaceId }),
        utils.research.spaces.listRunLinks.invalidate({
          spaceId,
          limit: 50,
          offset: 0,
        }),
