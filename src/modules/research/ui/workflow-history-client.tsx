...
import { buildWorkflowRunHistoryItems } from "~/shared/ui/navigation/workspace-history";
...
const historyHrefBySection: Record<WorkspaceSection, string | undefined> = {
  ...
  workflows: "/research/history",
  timing: "/timing/history",
  watchlists: undefined,
  companyResearch: "/research/company/history",
  spaces: undefined,
};
...
export function WorkflowHistoryClient(props: {
  ...
  const runsQuery = api.research.runs.listRuns.useInfiniteQuery(
    {
      limit: pageSize,
      templateCode,
      templateCodes,
  ...
  const cancelMutation = api.research.runs.cancelRun.useMutation({
    onSuccess: async (_, variables) => {
      await Promise.all([
        utils.research.runs.listRuns.invalidate(),
        utils.research.runs.getRun.invalidate({ runId: variables.runId }),
      ]);
    },
  });
  ...
  const runDetailQuery = api.research.runs.getRun.useQuery(
    { runId: selectedRunId ?? "" },
    {
            ...
            <>
              <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[var(--app-border)] pb-5">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Link
                    href={`/research/spaces?addRunId=${runDetailQuery.data.id}`}
