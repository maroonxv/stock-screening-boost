...
vi.mock("~/modules/screening/ui/stock-search-picker", () => ({
...
vi.mock("~/shared/ui/navigation/workflow-stage-switcher", () => ({
...
vi.mock("~/modules/timing/ui/timing-signal-card-list", () => ({
  TimingSignalCardList: () => React.createElement("div", null, "signal-cards"),
}));

vi.mock("~/modules/timing/ui/timing-stage-tabs", () => ({
  timingStageTabs: [
    {
      id: "results",
      label: "查看建议",
  ...
  return {
    api: {
      useUtils: () => ({
        ...
        research: {
          runs: {
            listRuns: { invalidate: vi.fn(async () => undefined) },
            getRun: { invalidate: vi.fn(async () => undefined) },
          },
        },
      }),
      watchlist: {
...
describe("TimingClient", () => {
  ...
  it("renders the watchlist timing workflow error when the combination suggestion run fails", async () => {
    ...
    const { TimingClient } = await import("~/modules/timing/ui/timing-client");
