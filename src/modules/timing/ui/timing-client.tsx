...
import { StockSearchPicker } from "~/modules/screening/ui/stock-search-picker";
import {
  DEFAULT_TIMING_PRESET_CONFIG,
  resolveTimingPresetConfig,
} from "~/modules/timing/server/domain/preset";
import { TimingSignalCardList } from "~/modules/timing/ui/timing-signal-card-list";
...
import {
  ...
  parsePortfolioForm,
  strategyStyleCards,
} from "~/modules/timing/ui/timing-wizard-view-models";
import { api } from "~/platform/trpc/react";
import { WorkflowStageSwitcher } from "~/shared/ui/navigation/workflow-stage-switcher";
import { buildTimingReportHistoryItems } from "~/shared/ui/navigation/workspace-history";
import {
  cn,
  EmptyState,
  InlineNotice,
...
export function TimingClient() {
  ...
  const startSingleMutation =
    api.research.runs.startTimingSignalPipeline.useMutation({
      onSuccess: async () => {
  ...
  const startWatchlistCardsMutation =
    api.research.runs.startWatchlistTimingCardsPipeline.useMutation({
      onSuccess: async () => {
        setAutoRefreshUntil(Date.now() + 3 * 60_000);
        setActiveTabId("results");
        await Promise.all([
  ...
  const startWatchlistTimingMutation =
    api.research.runs.startWatchlistTimingPipeline.useMutation({
      onSuccess: async () => {
        setAutoRefreshUntil(Date.now() + 3 * 60_000);
        setActiveTabId("results");
        await Promise.all([
  ...
  const startTimingReviewLoopMutation =
    api.research.runs.startTimingReviewLoop.useMutation({
      onSuccess: async () => {
        setAutoRefreshUntil(Date.now() + 3 * 60_000);
        await Promise.all([
          utils.timing.listTimingCards.invalidate(),
